function result(a) {
    var p = 0,
        ...
        l = [];
    for (let n in data)
        for (let g in data[n].list)
            if (a == data[n].list[g].name){
                var d = data[n].type,
        ...
        if (searchHash = location.hash.substring(3), "" != searchHash)
            for (let a in data)
                for (let i in data[a].list)
                    if (searchHash == data[a].list[i].no){
                      result(data[a].list[i].name);
                      search.val(data[a].list[i].name);
                      return 0;
                    }
        search.val("");
